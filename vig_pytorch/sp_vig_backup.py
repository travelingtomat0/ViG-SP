...
class modDeepGCN(torch.nn.Module):
    ...
    def forward(self, input):
        ...
        img_b, result_tuples_b, index_arrays_b, num_pixels_list, num_sp_list = input
        ...
        #print(f"shape of catted index arrays {catted_index_arrays.shape}")
        catted_index_arrays = catted_index_arrays.cuda()
        

        x = torch.index_select(conv_features, 1, catted_index_arrays)
        #print(f"shape of index_select x {x.shape}")
        ...
        #print(f"catted pixels list len is {len(num_pixels_list)}")
        
        splitted = torch.split(x, num_pixels_list, dim=0)
        #print(f"len of splitted {len(splitted)}")

        #num_sp_list = [len(b) for b in result_tuples_b]
        #print(f"num sp list {num_sp_list}")

        #print(f"RESULT TUPLES B LIST LENGTH {len(result_tuples_b)}")
        #print(f"RESULT TUPLES B FE {result_tuples_b[0].size()}")

        #print(f"len of result tuples is {len(result_tuples)}")
        ...
        for r_t, real_features in zip(result_tuples_b, splitted):
            edge_index = r_t
            #edge_index = torch.from_numpy(r_t.T)
            # print(real_features.shape)
            # print(type(real_features))
            # print(real_features.is_cuda)

            # print(edge_index.shape)
            # print(type(edge_index))
            # print(edge_index.is_cuda)

            data = Data(x=real_features, edge_index=edge_index)
            data = self.transform(data).cuda()
            #print(f"is cuda edge index {edge_index.is_cuda}")
            #print(f"is cuda real features {real_features.is_cuda}")
            #print(f"is cuda {data.is_cuda}")
        ...
        # num_sp_list = []
        # real_features_list = []
        # for i, result_indices in enumerate(result_indices_b):
        #     #image level
        #     rf_list = []
        #     num_sp_list.append(len(result_indices))
        #     for idx, r_i in enumerate(result_indices):
        #         print(f" image {i} superpixel {idx} of {len(result_indices)}")
        #         #superpixel level
        #         num_pixels = len(r_i)
        #         real_feature_array = torch.empty((num_pixels, self.conv_out_dim), dtype=torch.float, device=torch.device('cuda'))
        #         print(f"does require grad {real_feature_array.requires_grad}")

        #         for count, pix in enumerate(r_i):
        #             y_ind = pix[0]
        #             x_ind = pix[1]
                
        #             real_feature_array[count, :] = conv_features[i, :, y_ind , x_ind]
                    
        #         print(f"does require grad {real_feature_array.requires_grad}")
        #         rf_list.append(real_feature_array)
        #     real_features_list.append(rf_list)
        
        # print("generated real features")

        # #batch level
        # graph_list = []
        # assert len(result_tuples_b) == len(real_features_list)
        # for result_tuples, real_features in zip(result_tuples_b, real_features_list):
        #     #image level
        #     assert len(result_tuples) == len(real_features)
        #     for r_t, r_f in zip(result_tuples, real_features):
        #         #superpixel level
        ...
        # print(f"generated graphs {len(graph_list)}")
