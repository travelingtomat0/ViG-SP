...
def vgnn_copy_feature_module(in_dim = 3, out_dim=192):
    #aggregator = aggr.MLPAggregation(in_channels=out_dim, out_channels=out_dim, max_num_elements=100, num_layers=1)
    print("Custom Aggregator")
    #aggregator = aggr.PowerMeanAggregation(p=1.0, learn = True, channels = out_dim)
    #mlp = models.MLP(in_channels=out_dim, out_channels=out_dim//2, act="leakyrelu", num_layers=1)
    #mlp2= models.MLP(in_channels=out_dim//2, out_channels=out_dim, act="leakyrelu", num_layers=1)
    #aggregator = aggr.DeepSetsAggregation(local_nn = mlp, global_nn = mlp2)
    # aggregator = aggr.MultiAggregation(aggrs=['median', 'min', 'max'], mode='attn', mode_kwargs={'in_channels':out_dim, 'out_channels':out_dim, 'num_heads':8})
